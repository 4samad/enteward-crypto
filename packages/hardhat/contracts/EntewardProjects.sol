...
contract EntewardProjects is ERC721, Ownable {
    ...
    enum ProjectStatus { Upcoming, Ongoing, Completed, Cancelled }
    ...
    struct ProjectDetails {
        ...
        string reportURI;    // URI to project report (used if project is Completed or Cancelled).
    ...
    event ProjectStatusUpdated(uint256 indexed tokenId, ProjectStatus newStatus, string reportURI);

    /// @notice Constructor sets the initial owner of the contract.
    /// @param initialOwner The address to set as the initial owner.
    constructor(address initialOwner) ERC721("EnteWardProjects", "EWP") Ownable(initialOwner) {}
    ...
    function updateStatus(uint256 tokenId, ProjectStatus newStatus, string memory reportURI) public onlyOwner editable(tokenId) {
        require(newStatus != ProjectStatus.Upcoming, "New status must be ongoing, completed or cancelled");
        require(newStatus == ProjectStatus.Ongoing || bytes(reportURI).length > 0,
